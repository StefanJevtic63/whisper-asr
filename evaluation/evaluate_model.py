...
def transcribe(whisper_asr, audio, language_code):
    """
    Transcribe and normalize the given audio using the Whisper ASR model.

    Args:
        whisper_asr (transformers.Pipeline): The Whisper ASR Hugging Face pipeline
        audio (dict): The audio data to transcribe
        language_code (str): Language code of the transcription language
...
def evaluation(dataset, whisper_asr, language_code):
    """
    ...
    Returns:
        ...
        references (list of str): List of reference transcriptions from the dataset
    """
    ...
    for item in tqdm(dataset, desc='Evaluating Progress'):
        ...
        references.append( normalizer(item[args.ref_key]) )  # normalize labeled text
...
def main(args):
    ...
    # evaluate
    predictions, references = evaluation(dataset, whisper_asr, args.language)
